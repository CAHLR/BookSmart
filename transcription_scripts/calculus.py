...
def process_exercise(exercise, solution_soup, section_title):
    problem_statements = []
    sibling = exercise.find_previous_sibling()
...
def question_transcription(chapter, input_file_path, solution_file_path, out_path, version="review"):
    ...
    if version == "test":
        ...
        sections = soup.find_all("div", {"data-type": "note"}, class_=lambda x: x and "ui-has-child-title" in x)
        title_tag = "h2"
    else:
    ...
    for section in sections:
        ...
        # Process the title element for MathML to LaTeX conversion
        if title_element:
            process_content(title_element)
            section_title = title_element.get_text(separator=" ", strip=True)
        elif version == 'review':
            section_title = "Review Exercises"
        else:
            section_title = "None"
...
def transcribe_lessons(in_prefix, out_prefix, ch):
    for i in np.arange(ch, ch+1, 0.1):
        chapter = round(i, 5)
