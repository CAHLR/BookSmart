...
def extract_and_process_text(element):
    if element:
        ...
        # Handle multiple choice ordered lists (forcing capital letters)
        for ol in element.find_all('ol'):
            choices = ol.find_all('li')
            for idx, choice in enumerate(choices):
                # Add "A.", "B.", etc. based on index
                choice.insert(0, f'{chr(65 + idx)}. ')  # 'chr(65)' is 'A'
...
def process_exercise(exercise, solution_soup, section_title):
    # Since the first problem might be mistakenly identified as a problem statement,
    # ensure that the problem statement is really meant to be a statement.
    problem_statement_tag = exercise.find_previous_sibling("p")

    # Check if the problem_statement_tag is not actually part of the question text
    if problem_statement_tag and not problem_statement_tag.find_next_sibling("div", {"data-type": "problem"}):
        problem_statement = extract_and_process_text(problem_statement_tag)
    else:
        problem_statement = "None"

    # Extract image link for the exercise itself
    image_link = extract_image_link(exercise, solution_soup) or "None"

    # Extract the question text
    question = extract_question_text(exercise)

    # Extract the problem ID and the corresponding answer
    ...
    return {
        "Topic": section_title,
        "Problem Statement": html.unescape(problem_statement),
...
def transcribe_reviews(in_prefix, out_prefix, problem_type, chapter):
